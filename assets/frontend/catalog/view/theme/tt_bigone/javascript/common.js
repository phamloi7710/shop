...
$(document).ready(function () {
    ...
    $('#search-by-category input[name=\'search\']').on('keydown', function (e) {
        if (e.keyCode == 13) {
            $('#search-by-category button#btn-search-category').trigger('click');
        }
    });
...
var cart = {
    'add': function (product_id, quantity) {
        $.ajax({
            ...
            dataType: 'json',
            beforeSend: function () {
                $('#cart-total .item-cart').button('loading');
            },
            complete: function () {
                $('#cart-total .item-cart').button('reset');
            },
            success: function (json) {
                ...
                if (json['success']) {
                    ...
                    setTimeout(function () {
                        $('#cart-total .item-cart').html(json['number']);
                    }, 100);
    ...
    'update': function (key, quantity) {
        $.ajax({
            ...
            success: function (json) {
                json['number'] = json['total'].split(' ')[0];
                /* Need to set timeout otherwise it wont update the total */
                setTimeout(function () {
                    $('#cart-total .item-cart').html(json['number']);
                }, 100);
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if (json['success']) {
                    ...
                    $('#wishlist-total > span').html(json['number']);
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if (json['success']) {
                    ...
                    $('#compare-total  > span').html(json['number']);
                    $('#compare-total').attr('title', json['total']);
                    $('#content #compare-total').html(json['total']);
